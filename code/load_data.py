...
def one_hot_target_sklearn(sequence, dictionary = aminoacids):
...
def one_hot_smiles_sklearn(sequence, dictionary = smiles_dic):
# Check and convert string to list of letters
	if type(sequence) is str:
		sequence = np.array(list(sequence)).reshape(-1,1)
	else:
...
def hot_target(sequence, maxLength):
	result = np.zeros(len((sequence), max Length))
# Load Data
